...
Defining a grid of points $\mathcal{X} \equiv \left[-2, 2 \right]$, and choosing values for $\alpha$ and $l$, we can sample vectors $\mathbf{f}$ from the GP prior $\mathcal{N}\left(\mathbf{0}, \mathbf{C} \right)$, where $\mathbf{C}_{ij} = k \left( \boldsymbol{x}_{i}, \boldsymbol{x}_{j} \right)$. 
